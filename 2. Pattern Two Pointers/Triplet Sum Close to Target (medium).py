...
def two_sum_close(arr, start, result, third, target):
    min_dist = result[0]
    min_sum = result[1]
    ...
    while left < right:
        ...
        if abs(target - (third + sum)) < min_dist and third + sum < min_sum:
            result[0] = abs(target - (third + sum))
            result[1] = third + sum

        if result[0] == 0:
            break
        elif third + sum < target:
            left += 1
        else:
            right -= 1
