...
def ans(arr, target):
    ...
    for i in range(len(arr)):
        if i > 0 and arr[i] == arr[i - 1]:
            continue
        results.extend(three_sum(arr, target, i))
    return results


def three_sum(arr, target, four):
    results = []
    for i in range(four + 1, len(arr)):
        if i > 0 and arr[i] == arr[i - 1]:
            continue
        results.extend(two_sum(arr, target, four, i))
    return results


def two_sum(arr, target, four, three):
    left, right = three + 1, len(arr) - 1
    results = []
    while left < right:
        sum = arr[left] + arr[right] + arr[three] + arr[four]
        if sum == target:
            results.append([arr[four], arr[three], arr[left], arr[right]])
            left += 1
            right -= 1
...
def main():
    print(ans([4, 1, 2, -1, 1, -3], 1))
    print(ans([2, 0, -1, 1, -2, 2], 2))
