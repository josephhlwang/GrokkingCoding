...
def ans(jobs):
    ...
    for job in jobs:
        while cur_jobs and cur_jobs[0].end <= job.start:
            temp = heappop(cur_jobs)
            cur_load -= temp.cpu_load

        heappush(cur_jobs, job)
...
def main():
    print(
        ...
        + str(ans([job(1, 4, 3), job(2, 5, 4), job(7, 9, 6)]))
    )
    print(
        "Maximum CPU load at any time: "
        + str(ans([job(6, 7, 10), job(2, 4, 11), job(8, 12, 15)]))
    )
    print(
        "Maximum CPU load at any time: "
        + str(ans([job(1, 4, 2), job(2, 4, 1), job(3, 6, 5)]))
    )
